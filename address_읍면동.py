...
df_all = pd.read_csv('./address_text.csv', encoding='utf-8') 
df = df_all[['Address', 'Name','Latitude','Longitude']] # 기관ID, 병원경도, 병원위도 열만 추출 (도로명주소는 일부러 날림)


for i in range(len(df)): 
    x_crd = float(df.loc[i, ['Longitude']])
    y_crd = float(df.loc[i, ['Latitude']])
...
df.to_csv('주소_읍면동.csv', index=False)
